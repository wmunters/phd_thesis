...
\begin{figure}
	\centering
	\includegraphics[width=0.945\linewidth]{chapters/philtrans_torque/figure6.eps}
...
\begin{figure}[t]
	...
	\caption[Dynamics of controlled wind farms.]{Dynamics of controlled wind farms. 
		\emph{Left (a,c): } Thrust oefficient of the central first-row wind turbine. Colors: $C_T'$, Grey: $\widehat{C}_T'$. \emph{Right (b,d): } Aggregate power extraction normalized by time average of reference power extraction. \emph{Top (a,b): } $C_T'^{max}=3$. \emph{Bottom (c,d): } $C_T'^{max} = 2$. \legendtauref \label{fig:time_control_and_time_power}}
\end{figure}
